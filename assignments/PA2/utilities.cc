...
void print_escaped_string(ostream &str, const char *s) {
  // The function print_escaped_string is used to print a string with all
  // necessary escape characters. This ensures that special characters within
  // the string are correctly represented in the output.
  while (*s) {
    switch (*s) {
      case '\\':
        str << "\\\\";
...
void dump_cool_token(ostream &out, int lineno, int token, YYSTYPE yylval) {
  // The function dump_cool_token is designed to output a readable
  // representation of a token, including its line number and semantic value
  out << "#" << lineno << " " << cool_token_to_string(token);
  // Token-Specific Handling
  // uses a switch statement to handle different token types and print their
  // specific semantic values.
  switch (token) {
    case (STR_CONST):
      out << " \"";
      // An escaped string is a string where certain characters are preceded by
      // a backslash (\) to indicate that they should be interpreted
      // differently.
      // The get_string() method is typically used to retrieve the actual string
      // value stored in a symbol. In the context of a compiler or lexer, a
      // symbol might represent an identifier, keyword, or literal value. The
      // get_string() method returns the string representation of that symbol.
